...
const controlPanelButtonHandlers = {
  ...
  'ticket_create': async (interaction) => {
    console.log('=== ticket_create ハンドラー開始 ===');
    console.log(`ユーザー: ${interaction.user.tag}, チャンネル: ${interaction.channel?.name}`);
    
    try {
      const settings = await getTicketSettings(interaction.guild.id);
      console.log(`チケット設定: enabled=${settings.enabled}, is_paid=${settings.is_paid}`);
      
      if (!settings.enabled) {
        console.log('チケットシステム無効');
        await interaction.reply({
      ...
      console.log(`チケットチャンネル作成: ${ticketChannel.id}`);
      
      await createTicketChannel(interaction.guild.id, ticketChannel.id, interaction.user.id, ticketNumber);
      console.log(`チケット記録作成: guildId=${interaction.guild.id}, channelId=${ticketChannel.id}`);
      ...
      console.log('チケット作成完了');
    } catch (error) {
      console.error('チケット作成エラー:', error.message, error.stack);
      try {
        if (interaction.deferred) {
          await interaction.editReply({
            content: `❌ チケットの作成に失敗しました: ${error.message}`
          });
        } else {
          await interaction.reply({
            content: `❌ チケットの作成に失敗しました: ${error.message}`,
        ...
        console.error('エラーメッセージの返信に失敗:', replyError.message);
      }
    }
  },
...
const controlPanelModalHandlers = {
  ...
  'modal_delete_keyword': async (interaction) => {
    ...
    const limit = Math.min(parseInt(interaction.fields.getTextInputValue('limit')) || 100, 1000);
    const serverWide = (interaction.fields.getTextInputValue('server_wide') || 'yes').toLowerCase() !== 'no';
    ...
    if (serverWide) {
      ...
      channels = Array.from(all.values()).filter(c => c && c.isTextBased && !c.isThread);
    }
    
    // 各チャンネルでメッセージを削除
    for (const channel of channels) {
      if (deleted >= limit || !channel) continue;
    ...
    await interaction.editReply({ content: `✅ ${deleted}件削除しました。` });
  },
